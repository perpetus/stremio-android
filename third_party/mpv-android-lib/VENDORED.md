...
- `cacert.pem`
- prebuilt release native libraries for `arm64-v8a` and `x86_64`
...
The script builds upstream for `arm64` and `x86_64`, then copies the generated `.so` files into this module, omitting `libc++_shared.so`.
